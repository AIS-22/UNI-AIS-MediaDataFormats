...
def decode_jpeg(enc_file, dec_file, subfolder_needed=True):
...
def encode_jpeg(printProgress=False, maxFileSizeKb=32, useMultiCropPerImage=False):
    i = 0
...
def encode_jpeg_q(image_path, decoded_path, q):
    ...
    decode_jpeg(outputPath, decoded_path, False)
